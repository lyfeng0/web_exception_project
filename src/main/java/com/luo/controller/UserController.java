...
public class UserController {
	...
	public ModelAndView getUpload(HttpServletRequest request) throws Exception {
		ModelAndView mav = new ModelAndView("upload");
		return mav;
	}
    
    @RequestMapping("/uploadfile.jhtml")  
    public ModelAndView upload(@RequestParam MultipartFile[] myfiles,HttpServletRequest request) throws IOException {  
    	DecriptUtil ut = new DecriptUtil();
        for(MultipartFile file : myfiles){     
            ...
            if(file.isEmpty()){   
                ...
                //得到服务器项目发布运行所在地址  
                String path1 = request.getSession().getServletContext().getRealPath("static")+File.separator;  
                ...
                try {
					ut.CreateFile(file.getInputStream(), path2);
				} catch (Exception e) {
